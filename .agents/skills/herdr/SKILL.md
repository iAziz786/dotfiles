...
The `HERDR_*` variables are per-pane, captured when the pane is spawned. They describe your own context — they do NOT change when the user or another client switches focus to a different workspace, tab, or pane. They are the only safe anchor for where you live. They can go stale only if your pane was moved to another workspace after spawn.

**Workspace/tab creation resolves against the UI-focused workspace, not your pane.** `herdr tab create` without `--workspace` lands the new tab in whatever workspace the user is looking at right now — if they switch mid-task, your tab lands there. `herdr workspace create` runs fine with defaults and creates a brand-new workspace; never run it unless the user explicitly asked for one. Always pass `--workspace "$HERDR_WORKSPACE_ID"` explicitly on every `tab create` and workspace-scoped command, and use `--tab "$HERDR_TAB_ID"` / `--current` where pane commands need a tab context. Never derive targets from `workspace list` ordering or "the focused/current workspace".
...
Default to a sibling pane in the current tab and the current working directory. Do not create a workspace, tab, worktree, or different cwd unless the user explicitly requests that topology or location. When a new tab IS required, create it explicitly in your own workspace — `herdr tab create --workspace "$HERDR_WORKSPACE_ID"` — and read the tab ID from `.result.tab.tab_id`. Never omit `--workspace`: the default is the UI-focused workspace, which follows the user's focus, not where your pane runs.
...
- Always pass `--workspace "$HERDR_WORKSPACE_ID"` explicitly when creating tabs or running workspace-scoped commands. Focus-based defaults belong to the user's current view, not your pane — the user switching workspaces mid-task must never move your work.
