---
name: playwright-cli
description: Automate browser interactions, test web pages, work with Playwright tests, and check Gmail emails.
...
- If you need a new session for a quick task while one is already open, use a named session:
  ```bash
  playwright-cli -s=quick-task open https://example.com
  ```
  Then close only that named session when done:
  ```bash
  playwright-cli -s=quick-task close
  ```
